...
export function tokei() {
  main_text_box.innerHTML = `とけい　<span style="font-size:12px;">とけいのはりは　スライダーでうごかせるよ。</span>`;
  //初期設定
  document.getElementById("sub").innerHTML = "";
  const tokei_board = document.createElement("div");
...
`;
  ...
  // const t_range = new KeenSlider(t_range);
  ...
  plus.addEventListener("click", () => {
    tokei_range.value = Math.floor(
      Number(tokei_range.value) + Number(tokei_range.step)
    );
    hours = Math.floor(tokei_range.value / 60);
    minutes = Math.floor(tokei_range.value % 60);
    data.kako.currentTime = 0;
  ...
  check.addEventListener("click", () => {
    ...
    if (hours == answer_hours && minutes == answer_minutes && flag == true) {
      ...
      (hours != answer_hours || minutes != answer_minutes) &&
      flag == true
    ) {
  ...
  function hRotate() {
    ...
    ctx.lineTo(
      200 +
        100 * Math.cos((Math.PI / 180) * (270 + 30 * (hours + minutes / 60))),
      200 +
        100 * Math.sin((Math.PI / 180) * (270 + 30 * (hours + minutes / 60)))
    );
    ctx.strokeStyle = "red";
    ctx.stroke();
  }
  ...
  function drawText() {
    ...
    const textArrY = [
      105, 150, 210, 275, 320, 335, 320, 270, 210, 150, 105, 85,
    ];
