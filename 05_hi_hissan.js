...
export function hihi() {
  ...
  document
    ...
    .addEventListener("click", () => masu_clear());
  ...
  document
    .getElementById("kotae")
    .addEventListener("click", () => show_answer());
    ...
    for (let col = 0; col < Math.max(higensu_keta, gensu_keta) - 1; col++) {
      if (Math.floor(higensu_arr[col] - gensu_arr[col] - kurisagari) < 0) {
        TBL.rows[0].cells[max_keta - col - 1].innerHTML =
          "<span style='color:red;font-size:20px;vertical-align:bottom;'>" +
          "10" +
          "</span>";
        TBL.rows[1].cells[max_keta - col - 2].innerHTML =
          ...
          higensu_arr[col + 1] +
          "</span>" +
          "<span style='color:red;font-size:20px;vertical-align:top;'>" +
          Math.floor(higensu_arr[col + 1] - 1) +
          "</span>";
        kurisagari = 1;
      } else kurisagari = 0;
      //２回繰り下がり　かつ　被減数の10の位が0の時
      if (
        Math.floor(higensu_arr[0] - gensu_arr[0]) < 0 &&
        higensu_arr[1] == 0
      ) {
  ...
  function kotae_input() {
    ...
    box5.value =
      ...
      Number(TBL.rows[3].cells[3].innerText) * 1;
    if (box5.value == sa) {
      box5.style.color = "red";
      data.seikai1.currentTime = 0;
      data.seikai1.play();
  ...
  document.addEventListener(
    ...
    function (event) {
      ...
      if (event.target.className == "droppable-elem") {
        ...
        event.target.className == "droppable-elem-2" &&
        dragged.tagName == "IMG"
      ) {
        dragged.parentNode.removeChild(dragged);
        event.target.appendChild(dragged);
  ...
  function img_kurisagari() {
    const img_arr = ["ichien", "juuen", "hyakuen"];
    for (let j = 0; j < 3; j++) {
      var count = TBL_2.rows[0].cells[3 - j].getElementsByClassName(
        img_arr[j + 1]
      ).length;
      if (count == 1) {
        data.reset.currentTime = 0;
        data.reset.play();
        TBL_2.rows[0].cells[3 - j]
          .getElementsByClassName(img_arr[j + 1])[0]
          .remove();
        img_style(j);
      }
    }
